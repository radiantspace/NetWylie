...
/* This C# namespace implements the conversion between Unicode Tibetan text, and
   Wylie (EWTS) transliteration.

   It is based on the equivalent Java package found at 
   http://www.digitaltibetan.org/tibetan/JavaWylie-dev.zip
   ...
   This code is a library; it is meant to be embedded in other software.
      ...
      using org.rigpa.wylie;
         ...
      Wylie wl = new Wylie();
      Console.WriteLine(wl.fromWylie("sems can thams cad"));
      Console.WriteLine(wl.toWylie("\u0f66\u0f44\u0f66\u0f0b\u0f62\u0f92\u0fb1\u0f66\u000a"));

   Right now the code exists as a Visual Studio solution.  I will work on a solution that does not
   require Visual Studio soon.
 
   There is only one unit test method, but it runs a series of 211 data-driven tests.
   ...
   As C# and Java are syntactically very similar, I tried to make as few changes to the actual
   code as possible so that it will be easy to track changes to the original Java and apply 
   those to the C# code.  To accomplish this, I made heavy use of C# extention methods where 
   the C# and Java syntax diverged.  Beyond those, a few global search-and-replace values got me
   most of the way there:
    ...
    Another major change was the use of labeled loops within the Java code which enabled breaking
    and continuing parent loops.  I could not find similar support within C#, so in some instances
    I had to add logic to support breaking out of a loop and continuing the parent loop.
    
    There are a few additional changes beyond that, but really not much.


   AUTHOR AND LICENSE
   ==================

   Copyright (C) 2015 Todd Gray
...
namespace org.rigpa.wylie
{
    ...
    private bool check, check_strict, print_warnings, fix_spacing;
    ...
    private static HashMap<char, String> m_tib_top, m_tib_subjoined, m_tib_vowel, m_tib_final_wylie, m_tib_final_class, 
					      m_tib_other;
    private static HashMap<String, int> m_ambiguous_key;
    private static HashMap<char, int> m_tokens_start;
	...
	// java mappings auto-generated from the Perl code
	...
	m_ambiguous_key = new HashMap<String, int>();
	...
	// top letters
	m_tib_top = new HashMap<char, String>();
	m_tib_top.put('\u0f40', 	"k");
	m_tib_top.put('\u0f41', 	"kh");
	m_tib_top.put('\u0f42', 	"g");
	m_tib_top.put('\u0f43', 	"g+h");
	...
	// subjoined letters
	m_tib_subjoined = new HashMap<char, String>();
	...
	m_tib_vowel = new HashMap<char, String>();
	m_tib_vowel.put('\u0f71', 	"A");
	m_tib_vowel.put('\u0f72', 	"i");
	m_tib_vowel.put('\u0f73', 	"I");
	m_tib_vowel.put('\u0f74', 	"u");
	...
	// final symbols => wylie
	m_tib_final_wylie = new HashMap<char, String>();
	...
	m_tib_final_class = new HashMap<char, String>();
	...
	m_tib_other = new HashMap<char, String>();
	m_tib_other.put(' ', 		"_");
	m_tib_other.put('\u0f04', 	"@");
	m_tib_other.put('\u0f05', 	"#");
	m_tib_other.put('\u0f06', 	"$");
	...
	m_tokens_start = new HashMap<char, int>();
    ...
    private void initWylie(bool check, bool check_strict, bool print_warnings, bool fix_spacing) {
    ...
    public Wylie(bool check, bool check_strict, bool print_warnings, bool fix_spacing) {
    ...
    private String consonant(String s) {
	return m_consonant.get(s);
    }

    private String subjoined(String s) {
    ...
    private String final_uni(String s) {
	return m_final_uni.get(s);
    }

    private String final_class(String s) {
	return m_final_class.get(s);
    }

    private String other(String s) {
    ...
    private bool isSuperscript(String s) {
	return m_superscripts.containsKey(s);
    }

    private bool superscript(String sup, String below) {
	HashSet<String> tmpSet = m_superscripts.get(sup);
	if (tmpSet == null) return false;
	return tmpSet.contains(below);
    }

    private bool isSubscript(String s) {
	return m_subscripts.containsKey(s);
    }

    private bool subscript(String sub, String above) {
	HashSet<String> tmpSet = m_subscripts.get(sub);
	if (tmpSet == null) return false;
    ...
    private bool prefix(String pref, String after) {
    HashSet<String> tmpSet = m_prefixes.get(pref); 
	if (tmpSet == null) return false;
	return tmpSet.contains(after);
    }

    private bool isSuffix(String s) {
	return m_suffixes.contains(s);
    }

    private bool isSuff2(String s) {
	return m_suff2.containsKey(s);
    }

    private bool suff2(String suff, String before) {
    HashSet<String> tmpSet = m_suff2.get(suff);
	if (tmpSet == null) return false;
	return tmpSet.contains(before);
    }

    private int? ambiguous_key(String syll) {
        if (m_ambiguous_key.ContainsKey(syll))
            return new int?(m_ambiguous_key.get(syll));
        else
            return null;
    }

    private String ambiguous_wylie(String syll) {
	return m_ambiguous_wylie.get(syll);
    }

    private String tib_top(char c) {
	return m_tib_top.get(c);
    }

    private String tib_subjoined(char c) {
	return m_tib_subjoined.get(c);
    }

    private String tib_vowel(char c) {
	return m_tib_vowel.get(c);
    }

    private String tib_vowel_long(String s) {
	return m_tib_vowel_long.get(s);
    }

    private String tib_final_wylie(char c) {
	return m_tib_final_wylie.get(c);
    }

    private String tib_final_class(char c) {
	return m_tib_final_class.get(c);
    }

    private String tib_caret(String s) {
	return m_tib_caret.get(s);
    }

    private String tib_other(char c) {
	return m_tib_other.get(c);
    }
    private bool tib_stack(String s) {
	...
	while (i < maxlen) {
	    ...
	    char c = str.charAt(i);
	    int? mlo = new int?();
        try { mlo = m_tokens_start.get(c); } catch { }

	    // if there are multi-char tokens starting with this char, try them
	    if (mlo != null) {
		for (int len = mlo.Value; len > 1; len--) {
		    if (i <= maxlen - len) {
	    ...
	    tokens[o++] = c.ToString(); //char.toString(c);
	    i += 1;
	}
	...
	while (tokens[i] != null) {
		...
		while (tokens[i] != null) {
		    ...
		    if (t.equals("[")) nesting++;
		    if (t.equals("]")) nesting--;
		    if (nesting == 0) { cont = true; break; } //continue ITER;
		...
		outt.append(tb.uni_string);
		i += tb.tokens_used;
		units++;

		foreach (String w in tb.warns) {
		...
		warnl(warns, line, "Unexpected char \"" + t + "\".");
	    }
	...
	if (units == 0) warn(warns, "No Tibetan chars found!");

	return outt.toString();
    }

    // does this string consist of only hexadecimal digits?
    private bool validHex(String t) {
	...
	return ((char)Int32.Parse(hex, NumberStyles.HexNumber)).ToString(); //char.valueOf((char)int.parseInt(hex, 16)).toString();
    }
	...
	if (this.print_warnings) Console.WriteLine(str); // System.out.println(str);
    }
	...
	Console.WriteLine(">>" + name + "<< : (" + (o == null ? "NULL" : o.toString()) + ")");
    }
    
    // join a list (ArrayList or LinkedList) of strings into a single string
    private String joinStrings(IEnumerable<String> a, String sep) {
	StringBuilder outt = new StringBuilder();
	int len = a.size();
	...
	foreach (String v in a) {
	    outt.append(v);
	    if (sep != null && i < len - 1) outt.append(sep);
	    i++;
	}
	...
	bool plus = false;		// any explicit subjoining via '+'?
	...
	while (true) {
	    ...
	    t = tokens[i];
	    if (consonant(t) != null || (outt.length() > 0 && subjoined(t) != null)) {
		if (outt.length() > 0) {
		    outt.append(subjoined(t));
		} else {
		    outt.append(consonant(t));
		}
		i++;
		...
		if (outt.length() == 0) outt.append(vowel("a"));
		if (!t.equals("a")) outt.append(vowel(t));
		i++;
		vowel_found = t;
	...
	if (consonants > 1 && vowel_found == null) {
		...
		outt.setLength(0);
		outt.append(consonant(single_consonant));
	    }
	}
	...
	if (this.check && warns.size() == 0 && check_root && root_idx >= 0) {
		...
		int? expect_key = ambiguous_key(cc);
		if (expect_key != null && expect_key.Value != root_idx) {
    ...
    //    escape: whether to escape non-tibetan chars according to Wylie encoding.
    ...
    public String toWylie(String str, List<String> warns, bool escape) {
	StringBuilder outt = new StringBuilder();
	int line = 1;
	int units = 0;
	...
	while (i < len) {
		...
		if (!escape) i += handleSpaces(str, i, outt);
		continue; // ITER;
	    }
	    ...
	    // other chars in the tibetan plane, escape with \\u0fxx
	    if (t >= '\u0f00' && t <= '\u0fff') {
		...
		// unicode-escape anything in the tibetan plane (i.e chars not handled by Wylie)
		} else if (t >= '\u0f00' && t <= '\u0fff') {
		    outt.append(formatHex(t));
    ...
    // given a char, return a string like "\\uxxxx", with its code in hex
    private String formatHex(char t) {
        // not compatible with GWT...
	// return String.format("\\u%04x", (int)t);

	StringBuilder sb = new StringBuilder();
	sb.append("\\u");
    String s = String.Format("{0:x4}", (int)t); // int.toHexString((int)t);
    ...
    private int handleSpaces(String str, int i, StringBuilder outt) {
	int found = 0;
	int orig_i = i;
    ...
    private bool followedByNonTibetan(String str, int i) {
	int len = str.length();
	...
	if (stacks.size() == 3 && stacks.get(0).prefix && stacks.get(1).suffix && stacks.get(2).suff2) {
	    ...
	    foreach(ToWylieStack st in stacks) {
		strb.append(st.single_cons);
	    }
	    String ztr = strb.toString();
	    int? root = ambiguous_key(ztr);
	    ...
	    stacks.get(root.Value).prefix = stacks.get(root.Value).suffix = false;
	    stacks.get(root.Value + 1).suff2 = false;
	}
	...
	StringBuilder outt = new StringBuilder();
	foreach (ToWylieStack st in stacks) {
	    outt.append(putStackTogether(st));
	}

	ToWylieTsekbar ret = new ToWylieTsekbar();
	ret.wylie = outt.toString();
	ret.tokens_used = i - orig_i;
	ret.warns = warns;
	...
	//   - top symbol
	//   - stacked signs (first is the top symbol again, then subscribed main chars...)
	...
	if (tib_stack(st.cons_str)) {
	    outt.append(joinStrings(st.stack, ""));
	} else {
	    outt.append(st.cons_str);
	}
	...
	if (!st.vowels.isEmpty()) {
	    outt.append(joinStrings(st.vowels, "+"));
	...
	// final stuff
	outt.append(joinStrings(st.finals, ""));
	if (st.dot) outt.append(".");
	...
	// bool, later set to true if this is a prefix, suffix, 2nd suffix, or if we need a dot as in "g.yag"
	public bool prefix, suffix, suff2, dot;
